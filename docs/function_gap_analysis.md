...
| 6 | Condition Wizard 条件体系 | 97% | L2 | 中间层（§30 已接全部参数族；hub-B 展示型子页=持续跟随，§0.4） |
...
**整体完整度 ≈98%**（12 域精确均值 98.4；满格 7 域 / 中间 5 域 /
差距 0 域）。**深度谱**：L3 ×5（数据/.s/网格/PK/FEM——五条字节级
证据链）+ L2 ×6 + L2–L3 ×1。深度 100% 口径（每域 ≥L2、格式/内核/
验证面 L3）达标。

> **v9.3 刷新（2026-09-05 弧线终核）**：本节为当日全部批次落地后的
> 实测快照——§30 I 系（I4 泄漏守卫/I1a Solver Control/I1b 专用页/
> I1c 三子系统/I2 DRIVER+CNRM/I3 长尾 14+6 命令/I5 区域标量族面板）
> + H3 plate_fin/H4 草图样条/H6 axis_plane 探针 + v9.2 D10/D12 复核
> 更正 + R3.5d 官方键三连（panel/sphere/case_cube/two_resistor 桥/
> axial_fan→FANV）。语料 195 命令 100% 发射；全量 **953 passed /
> 5 skipped**（139 测试文件）。豁免清单 §0.3 与 NYI_INVENTORY
> （11 项）经核仍准确。
...
UI / 几何建模 / CW 条件 / 导入导出四域的 L3 升级路径、验收口径、
波次与禁区见 [L3_DEEPENING_PLAN.md](L3_DEEPENING_PLAN.md)
（2026-09-05 立项；波次 1 = 语料 parity 驱动器 + 官方零件保真矩阵）。

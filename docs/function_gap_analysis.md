...
| 10 | FEM | 75% | L1–L2 | 差距层（壳/六面体 kind 未实现，官方面已探针证实） |
| 11 | 高级工具 | 95% | L2 | 中间层 ▸边界项（WindTool/PICLS 黑盒参数深证） |
| 12 | 导入导出格式 | 93% | L2 | 中间层（IFC 导出仅矩形 profile；STEP/SAT 仅导入） |
...
**整体完整度 ≈95%**（12 域均值；满格 5 域 / 中间 6 域 / 差距 1 域）。
**深度谱**：L3 ×4（数据/.s/网格/PK——四条字节级证据链）+ L2 ×7 +
L1–L2 ×1。深度 100% 口径（每域 ≥L2、格式/内核/验证面 L3）下仅 FEM
未达 L2（tet4 参数闭环 L1、e2e L2、壳/六面体 L0）。
...
| D6 | 区域标量族（o2/n2/h2/vofl/trt2/tret/tpor）与部分头标志（vfre/wlty/vfgo）为存储级发射，无 CW 面板 | 下一 UI 批 |

"""Probe PK_MESH_create_from_facets callback ABI (V37) in isolation.

Run as a subprocess; a wrong ABI crashes only this process.
"""
import ctypes as C
from ctypes import (
    CFUNCTYPE, POINTER, Structure, byref, c_int, c_double, c_void_p, cast,
)
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT))
import ps_facet2_nodes as _ps

sess = _ps._get_session()
pk = sess.pk

# --- callback: log the incoming values --------------------------------
CALLS = []

@CFUNCTYPE(c_int, c_int, c_int, c_void_p, c_void_p, c_int, c_void_p, c_void_p)
def facet_reader(is_first, n_verts, coords, normals, n_facets, facet_data, ctx):
    CALLS.append((is_first, n_verts, n_facets))
    # peek at coords as doubles
    if n_verts > 0 and coords:
        d = cast(coords, POINTER(c_double))
        CALLS.append(("coord0", d[0], d[1], d[2]))
    if n_facets > 0 and facet_data:
        f = cast(facet_data, POINTER(c_int))
        CALLS.append(("facet0", f[0], f[1], f[2]))
    return 1  # PK_MESH_cb_status_stop_c ?

# --- options struct (V35 layout guess) --------------------------------
class _Box(Structure):
    _fields_ = [("x", c_double * 2), ("y", c_double * 2), ("z", c_double * 2)]

class _MeshCreateOpts(Structure):
    _fields_ = [
        ("o_t_version", c_int),
        ("vertices_estimate", c_int),
        ("facet_estimate", c_int),
        ("facet_free", c_void_p),
        ("create", c_int),
        ("have_box", c_int),
        ("box", _Box),
        ("thread_safe", c_int),
    ]

opts = _MeshCreateOpts()
C.memset(byref(opts), 0, C.sizeof(opts))
opts.o_t_version = 1
opts.create = 1   # guess: PK_MESH_create_now_c
mesh = c_int(0)

pk.PK_SESSION_set_check_arguments(0)
pk.PK_MESH_create_from_facets.restype = c_int
pk.PK_MESH_create_from_facets.argtypes = [
    c_void_p, c_void_p, POINTER(_MeshCreateOpts), POINTER(c_int)]
rc = pk.PK_MESH_create_from_facets(facet_reader, None, byref(opts), byref(mesh))
print("rc =", rc, "mesh =", mesh.value)
print("calls:", CALLS)

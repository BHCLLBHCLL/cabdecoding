...
* Table tokens (fctab) observed on this kernel (the V35 documentation
  order and the kernel's own V5 enum differ in the middle block):
    ...
    0x57B6 fin_data       0x57BB point_vec       0x57C1 curv_dirs
                         0x57BC normal_vec       0x57C2 facet_face
                                                 0x57C3 strip_face
...
* Data encodings observed on this kernel (V35 header ``pk_topol_fctab_*_t``):
  ...
  - facet_face:      8-byte records ``{int face; int data_or_fin}``
...
CHOICE_OFFSET = {
    ...
    "deriv_d2p": 0x146, "curv_dirs": 0x147, "facet_face": 0x148,
    "strip_face": 0x149,
}
...
class _PsSession:
    ...
    def facet2(self, tag: int, *,
        ...
        rc = pk.PK_TOPOL_facet_2(1, (c_int * 1)(tag), None, byref(opts),
                                 byref(result))
...
def tessellate_xt(xt_bytes: bytes, **kw) -> list[TessPart]:

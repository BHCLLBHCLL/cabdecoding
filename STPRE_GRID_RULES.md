...
### 2.3.1 tr03 representative 子集（P0-① 已解，2026-08-16）

**判据已解**：rep 模式投影**完全位于计算域盒内的 B-rep 顶点**（3D 判定，
与 all 模式的显示网格节点裁剪同规则）+ 部件 AABB 极值。此前成谜的
"被丢弃值"（-17.577、-8.103、+2.349…）全部追溯到其唯一来源顶点位于
域外（如 z=-8.103 的顶点 y=-29.05 在域盒外）——不存在按凸性/面类型的
选择规则（PK_EDGE_ask_convexity / PK_FACE_ask_type 均已实测排除）。

e2e 收敛（tools/check_all_mode.py，2026-08-16）：

| 模式 | native | golden | |
|---|---|---|---|
| all | (59, 118, 121) | (59, 118, 121) | MATCH |
| rep | (57, 91, 92) | (57, 91, 92) | MATCH |

历史排查记录（判据解开前的过程数据，保留供参考）：

- 域外顶点直接丢弃（部件 z < -20 的 23 个值全部不出现）。
- 域内值中阈值 0.1 内的并入邻近网格线（-19.989/-19.926 → -20 等）。
- 阈值联动：thr 0.1→92、0.5→89、2.0→42（z 轴），阈值=合并容差。
- 外部区（部件 AABB 顶面 47.4651 → 域顶 120）为 ratio_out 1.2 的几何级数
  12 段 {1.0,1.18,1.39,…,10.07}。

工具：`tools/reprobe_tr03.py`（live STpre COM 重跑 vd_0/vd_1，与金标一致）。

### 2.3.1a 细分计数的 float32 2/3 常数（P0-① 已解，2026-08-16）

内区等分计数 `n = floor(L/std + 2/3)` 的加数是 **float32 舍入值**
（C++ `2.0f/3.0f` 提升为 double = 0.6666666865348816…）：tr03 x 轴
区间 L=40/3（double 13.333333333333305）STpre 得 n=14；若用精确 double
2/3，和为 13.999999999999972 → 截断成 13（off-by-one）。float32 常数把
判别边界推到 L/std > 13.33333331…，与全部金标吻合（diag_all_diff85；
此前 40/3→14 的观察亦兼容）。实现：`cab_grid._equal_split`。

断言固化：`tests/test_golden_reference.py::
test_tr03_native_grid_counts_match_golden`（pskernel 缺失时 skip）。
...
- **facet 参数管线全解码（round 30，P0-1 突破：三角化器 = PK 本体）**：
  ...
  * **两处 PK 调用**：PK_TOPOL_facet（6 参 V35 数组签名，@0x1b45b2，
    ...
    化器——网格 = Parasolid 原生 faceter + STpre 容差配方。
  * **0x1b8a30 = PK_BODY_check 校验包装**（thunk 0x3e8b9e→3f59f0），
    在 facet_2 成功后跑检查，遍历 tag=0x35fa 的 0x28 字节记录
  ...
  * **容差配方（0x1b5620 路径，part 类型 >1 的通用面/实体）**：
    ...
    curve_chord_tol = D×(env[0x29C0]|0.001)；
    surface_plane_tol = D×(env[0x29C8]|0.001)；
    各积钳制 ≥1e-8；类型 1（圆柱轴向）走 0x1b5e80 变体
    （15°/30° + 0x1b5c00 网格步长 0.2D/0.001D）。
  ...
  * **结论（替代旧结论）**：all 顶点源=显示网格（已证）；显示网格 =
    ...
    （不再需要逐行移植 STpreBase）；下一步用 Python ctypes 直调
    pskernel 验证顶点计数对齐金色 59×118×121。env 五字段若出现在
    项目 XML/设置文件即可直接读取（当前模板未见，进程内全局）。
### 2.4 内区划分

...
| F3 非 Condition 对话框批 | ✅（6/7 族） | `99f9ea0` | 新 cab_extra_dialogs：Chemical Material / Compressible Fluid / Cloth Model / Check Time Step / Calculate Conductivity / Calculate HTC / Humidity φ-h 七对话框＋Option 菜单注册；AC Unit 5 机型仍开放（cab_parts 参数面） |
| F4 网格负面结论批 | ✅（2/4，余探针） | `566ff29` | STL/polygon relay body_files 修复（无效网格化根因）；multiblock×cylindrical 引擎级测试锁定；threshold/axis_plane 曲面语义仍需黑盒探针 |
...
无窗口主线进度：F1 ✅ F2 ✅ F3 ✅ F4 ✅ F5 ✅（5/5），全量 **786 passed / 5 skipped**。剩余 100% 路径：F6 许可窗口批（COM B 层 / FEM kind / PICLS）＋ F7 终审（727 页对照 → v7.0）；另 AC Unit 5 机型与三项曲面网格语义探针为残项。
...
## 27. v7.0 后刷新：残余差距与 100% 对齐终规划（2026-09-02）

> 基线：HEAD `1609216`，全量 **808 passed / 6 skipped**。
> §22 P1–P8、§23 C1–C8、§24 F1–F5/AM/MB、§25 F1–F5、§26 G1–G7 全部
> 完成。Solver_eng 手册逐命令对照 + 8 官方样本 G7 对拍已入仓。

### 27.1 v7.0 后的真实状态（去虚报审计）

v7.0 的「100%」按 §22.0 A/B/C 口径达成。以下按维度刷新，标注
**当前真实百分比**（B/C 定档不计入百分比，但计入"闭合"）：

| 维 | 百分比 | 闭合口径 | 残余（非 B/C） |
|---|:---:|---|---|
| D1 数据层 | **95%** | A | 深字段滚动（各专用件参数面逐字段对齐） |
| D2 几何 Part | **94%** | A | AC 机型=ac_type 已确认；深字段滚动同 D1 |
| D3 UI | **96%** ↑ | A | F3 七对话框落地；长尾子菜单图标微调 |
| D4 .s 导出 | **97%** ↑ | A+B | 33 section；G7 对拍 1 match + 10 diff（结构对齐，内容差因模型规模不同） |
| D5 网格 | **98%** ↑ | A+B | F4 polygon relay 根因修复；threshold 曲面探针解除负面结论 |
| D6 CW | **96%** ↑ | A+B | C1–C8 全闭合 + 708/708 页映射；wizard 子页 hub-B |
| D7 PK | **95%** | A+B | Edit Solid 12 型 + PK 六算子；draft/midsurface B |
| D8 求解闭环 | **92%** ↑ | A+B | F5 收敛缩放/导出；.pst 解析 B 不做 |
| D9 COM | **96%** ↑ | A 100% + B 236 项 | 沙箱 440 项、45 项参数依赖、Table 无数据 |
| D10 FEM | **100%** | A+B | 离线 tet4 + STpre 行为一致实证 |
| D11 工具 | **100%** | A+B | 四工具全启动；PICLS 工程参数 |
| D12 导入导出 | **93%** | A+B | SAT/MDL B 定档；CGNS 非对标 |

**加权总体 ≈96%**（v7.0 的 100% 含 B/C 闭合；此表仅计 A 级代码实现，
B/C 定档另列——两条口径并存，均为诚实度量）。

### 27.2 从 ≈96% 到 A 级 100% 的残项清单

按影响排序，全部无需许可窗口：

| # | 残项 | 维 | 影响 | 规模 |
|---|---|:---:|---|:---:|
| R1 | **G7 十个 diff section 逐项对齐** — PCLE_CREATE(60vs53)、ES_FIELD_BC(35vs22)、ES_FIELD(65vs21)、PCLE_HANDLING(46vs39)、LSOL_OPTION(44vs28)、LSOL_TIME_STEP(38vs14)、LSOL_FORCE_IP(29vs85)、SURF_POROUS(32vs25)、MOVB_CONTROL(36vs91)、DYNA_MOTION(30vs85) | D4 | 官方 cab 含更多条件/部件导致行差；逐项调试提取子集对拍 | L |
| R2 | **D9 第三批** — Property/Table 带材料名参数化、45 项参数依赖成员逐个补参、破坏性成员沙箱批量隔离（脚本化 temp 副本逐调用） | D9 | 终证率 236→600+ | M–L |
| R3 | **D1/D2 深字段滚动** — 各专用件参数面逐字段与 Pre_eng 手册对齐（AC Unit 内部结构、Peltier 热电参数、Delphi 网络拓扑…） | D1/D2 | 每字段 XML 往返测试 | M |
| R4 | **D6 partial 页补全** — 19 页部分覆盖条件（ porous 子类型、Ventilation 贡献率、Bubble Nucleus、MO 传质等）补全到全页覆盖 | D6 | CW 页/存储/发射 | M |
...
```
Phase 1（无窗口，最高优先）：R1 逐 section 对拍调试 → D4 → 100%
Phase 2（无窗口）：R2 COM 第三批 → D9 终证率 → 100%
Phase 3（无窗口）：R3+R4 深字段/partial 页 → D1/D2/D6 → 100%
Phase 4（可选）：R5 .pst 逆向 + R6 axis_plane → D8/D5 → 100%
```

### 27.4 100% A 级口径的达成条件

- **D4**: R1 完成——33 个 section 全部通过官方样本逐字节对拍
- **D9**: R2 完成——终证 ≥600/719（83%+），剩余 C 声明
- **D1/D2/D6**: R3+R4 完成——无"rolling"残余
- **D8**: R5 完成（或维持 B 不做声明）
- **D5**: R6 完成（或维持 B 声明）
- **D7**: 维持 B（内核限制，非代码缺口）
- **D10/D11**: 已闭合
- **D3**: 维持（菜单/对话框已覆盖，长尾图标不影响）

**R1–R4 全部完成后：12 维 A 级口径 ≈100%，加 B/C 定档附录 = 覆盖度
与深度双 100%。** 预计工作量 ≈2–3 个完整会话。

### 27.5 不做项（终态声明）

| 项 | 维 | 理由 |
|---|---|---|
| Sketch 约束求解器 | D2 | STpre 无此功能（C 超越项） |
| draft/midsurface PK | D7 | pskernel 无导出（内核实证 5000/5022） |
